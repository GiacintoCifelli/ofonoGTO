...
static char *GRIL_HEX_PREFIX[] = {"Device 0: ", "Device 1: "};
...
static int ril_create(struct ofono_modem *modem, enum ofono_ril_vendor vendor)
{
...
static int create_gril(struct ofono_modem *modem)
{
	...
	ip = getenv("OFONO_KRIL_IP");
	if(!ip)
		ip = "192.168.225.1"; // "127.0.0.1"
	portstr = getenv("OFONO_KRIL_PORT");
	if (portstr != NULL && *portstr != '\0') {
		int testport;
		char *endp;
		testport = (int) strtoul(portstr, &endp, 10);
		if (*endp == '\0')
			port = testport;
	}

	ofono_info("Using %s:%d as socket for slot %d.", ip, port, slot_id);

	rd->ril = g_ril_new_with_ip(ip, port, OFONO_RIL_VENDOR_K);
	...
	DBG();
	g_ril_register(rd->ril, RIL_UNSOL_RIL_CONNECTED, ril_connected, modem);

	g_ril_register(rd->ril, RIL_UNSOL_RESPONSE_RADIO_STATE_CHANGED, ril_radio_state_changed, modem);
...
OFONO_PLUGIN_DEFINE(kril, "KRIL modem driver", VERSION,
